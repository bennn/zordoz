...
(require zordoz/typed/typed-zo-structs
         (only-in racket/port with-input-from-bytes port->bytes))

(require/typed compiler/zo-parse
  [zo-parse (-> Input-Port compilation-top)])

(require/typed compiler/decompile
  [decompile (-> compilation-top Any)])

(require/typed compiler/zo-marshal
  [zo-marshal (-> compilation-top Bytes)])

;; =============================================================================

(: syntax->zo (-> (Syntaxof Any) compilation-top))
...
(: syntax->decompile (-> (Syntaxof Any) Any))
(define (syntax->decompile stx)
  (decompile (syntax->zo stx)))

(: zo->compiled-expression (-> compilation-top Any))
